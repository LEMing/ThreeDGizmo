...
describe('CubePartFactory', () => {
  beforeEach(() => {
    const mockCanvas = document.createElement('canvas') as jest.Mocked<HTMLCanvasElement>;
    const mockContext = {
      fillStyle: jest.fn(),
      fillRect: jest.fn(),
      fillText: jest.fn(),
  ...
  describe('createWireframe', () => {
    it('should create a wireframe with correct properties', () => {
      ...
      expect((wireframe.material as THREE.LineBasicMaterial).color.getHex()).toBe(CUBE_CONSTANTS.LINE_COLOR);
    });
  });
  ...
  describe('createEdgeBox', () => {
    it('should create an edge box with correct properties', () => {
      ...
      const color = (edgeBox.material as THREE.MeshStandardMaterial).color.getHexString().toUpperCase();
      expect(`#${color}`).toBe(CUBE_CONSTANTS.EDGE_COLOR);

      const geometry = edgeBox.geometry as THREE.BoxGeometry;
      expect(geometry.parameters.width).toBe(CUBE_CONSTANTS.CUBE_SIZE - CUBE_CONSTANTS.EDGE_SECTION_SIZE);
  ...
  describe('createCornerCube', () => {
    it('should create a corner cube with correct properties', () => {
      ...
      const color = (cornerCube.material as THREE.MeshStandardMaterial).color.getHexString().toUpperCase();
  ...
  describe('createFace', () => {
    it('should create a face group with correct properties', () => {
      ...
      const color = (faceMesh.material as THREE.MeshStandardMaterial).color.getHexString().toUpperCase();
      ...
      expect((textPlane.material as THREE.MeshStandardMaterial).map).toBeTruthy();
    });
  });
});
